...
// Проверка результата:
// Для любых имени и фамилии в любом регистре должны выводиться такие же имя и фамилия, но первая буква у них большая,
// а все остальные — маленькие.

// Критерии оценки:
// Код корректно выводит все сообщения в зависимости от значения переменных name и surname.

// Рекомендации к выполнению:
// Для получения куска строки можно воспользоваться конструкцией str.substr(from, length), где str — название переменной с
// исходной строкой, вместо from подставляется номер символа, после которого нужно взять кусок (0, если нужно брать с начала строки,
// 1 — после первого символа и т.д.), а вместо length — количество символов. При этом length можно опустить,
// если нужно взять всю оставшуюся строку. Конструкцию можно присвоить переменной, с которой потом можно работать как с обычной строкой.
// Для преобразования букв строки к верхнему регистру можно воспользоваться конструкцией str.toUpperCase(),
// а к нижнему — str.toLowerCase(). Результат аналогично можно присвоить переменной.

let _name = 'jOhn';
let surname = 'dOe';

let new_name = _name.substr(0, 1).toUpperCase() + _name.substr(1).toLowerCase();
